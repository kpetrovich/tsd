namespace CodeBetter.Json
{
    ...
    public class JsonSerializer
    {
        ...
        private void SerializeValue(string name, object value)
        {
            ...
            if (value is int || value is long || value is short || value is float || value is byte || value is sbyte || value is uint || value is ulong || value is ushort || value is decimal)
            {
            ...
            if (value is double)
            {
                _writer.WriteRaw(value.ToString().Replace(',','.'));
                return;
            }
